-- Mirrors Flyway V1__init.sql for manual/DBA use.
...
CREATE INDEX IF NOT EXISTS idx_medicine_trgm_desc
    ON medicine USING gin (allen_description gin_trgm_ops);

CREATE TABLE IF NOT EXISTS kent_rubric (
    id               BIGSERIAL PRIMARY KEY,
    chapter          VARCHAR(64),
    rubric_path      TEXT,
    rubric_text      TEXT,
    medicine_abbrev  VARCHAR(64),
    grade            SMALLINT
);
CREATE INDEX IF NOT EXISTS idx_rubric_med ON kent_rubric (medicine_abbrev);
CREATE INDEX IF NOT EXISTS idx_rubric_chap ON kent_rubric (chapter);
